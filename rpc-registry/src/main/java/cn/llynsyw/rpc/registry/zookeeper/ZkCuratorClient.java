...
public class ZkCuratorClient {
	//zk的客户端工具Curator（在本类通过new实例化的是，自动start）
	...
	public ZkCuratorClient(ZookeeperConfig config) {
		...
		StringBuilder zkPeers = new StringBuilder();
		for (String zkPeer : config.getPeers()) {
			zkPeers.append(zkPeer).append(",");
		}
		zkPeers.deleteCharAt(zkPeers.length() - 1);
		try {
			client = CuratorFrameworkFactory.builder()
					.connectString(zkPeers.toString())
